...
yum remove -y kernel*`uname -r`
echo ">>> Getting Kernel"
yum install -y kernel-devel kernel-headers gcc dkms make bzip2 perl libX11
ldconfig
...
wget -N http://download.virtualbox.org/virtualbox/$VBOX_VERSION/VBoxGuestAdditions_$VBOX_VERSION.iso
